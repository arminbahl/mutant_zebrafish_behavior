...
# Take parameters from model simulation script
tau = 0.8
noise_sigma = 10
T = 1
...
####
# Same direction
p0 = myfig.Plot(fig, num='d', xpos=16, ypos=20.5, plot_height=1.25, plot_width=1.3,
                           errorbar_area=False, lw=1,
...
df_binned_features_heading_angle_change_histograms, \
df_binned_features_inter_bout_interval_histograms, \
df_gmm_fitting_results = get_target_result(root_path / "surrogate_fish2" / "all_data_best_model_repeat1.h5", "wt")
...
####
# Turning histograms
p0 = myfig.Plot(fig, num='d', xpos=12, ypos=15.5, plot_height=2.75, plot_width=1.8,
                           errorbar_area=False, lw=1,
                           xl="Turn angle (deg)", xmin=-101, xmax=151, xticks=[-100, -50, 0, 50, 100, 150], hlines=[50],
                           yl="Probability density", ymin=-0.01, ymax=0.4,
                           yticks=[], vlines=[0])
...
####
# Same direction
p0 = myfig.Plot(fig, num='d', xpos=16, ypos=15.5, plot_height=2.75, plot_width=1.3,
                           errorbar_area=False, lw=1,
                           xl="Time since last turn (s)", xmin=-0.15, xmax=1.6, xticks=[0, 0.5, 1, 1.5], hlines=[50],
                           yl="Probability to turn\nin same direction (%)", ymin=44, ymax=91,
                           yticks=[50, 70, 90], vlines=[0])
...
df_estimated_parameters_model1 = pd.read_hdf(root_path / "surrogate_fish2" / "estimated_model_parameters.h5", key="data").query("genotype == 'wt'").droplevel(["genotype"])
df_estimated_parameters_model2 = pd.read_hdf(root_path / "surrogate_fish2" / "estimated_model_parameters.h5", key="data").query("genotype == 'het'").droplevel(["genotype"])
df_estimated_parameters_model3 = pd.read_hdf(root_path / "surrogate_fish2" / "estimated_model_parameters.h5", key="data").query("genotype == 'hom'").droplevel(["genotype"])
df_estimated_parameters_model4 = pd.read_hdf(root_path / "surrogate_fish1" / "estimated_model_parameters.h5", key="data").query("genotype == 'hom'").droplevel(["genotype"])

df_minimal_error_gen0_model1 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'wt' and generation == 0").droplevel(["genotype", "generation"])
df_minimal_error_gen0_model2 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'het' and generation == 0").droplevel(["genotype", "generation"])
df_minimal_error_gen0_model3 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'hom' and generation == 0").droplevel(["genotype", "generation"])
df_minimal_error_gen0_model4 = pd.read_hdf(root_path / "surrogate_fish1" / "errors_over_generations.h5", key="data").query("genotype == 'hom' and generation == 0").droplevel(["genotype", "generation"])

df_minimal_error_gen79_model1 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'wt' and generation == 79").droplevel(["genotype", "generation"])
df_minimal_error_gen79_model2 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'het' and generation == 79").droplevel(["genotype", "generation"])
df_minimal_error_gen79_model3 = pd.read_hdf(root_path / "surrogate_fish2" / "errors_over_generations.h5", key="data").query("genotype == 'hom' and generation == 79").droplevel(["genotype", "generation"])
df_minimal_error_gen79_model4 = pd.read_hdf(root_path / "surrogate_fish1" / "errors_over_generations.h5", key="data").query("genotype == 'hom' and generation == 79").droplevel(["genotype", "generation"])
